...
def evaluate_hand(hand_cards):
    ...
    match (is_flush(suits), is_straight(ranks), most_common[0][1], most_common[1][1] if len(most_common) > 1 else 0):
        case (True, True, _, _):
            # Royal Flush or Straight Flush
            sorted_cards = sorted(hand_cards, key=lambda x: rank_values([x.rank])[0], reverse=True)
        ...
        case (_, _, 2, 2):
            ...
            if rank_values([pair1[0].rank])[0] > rank_values([pair2[0].rank])[0]:
                ...
                pairs = pair2 + pair1 
            kickers = [card for card in hand_cards if card.rank != most_common[0][0] and card.rank != most_common[1][0]]
...
def best_hand(hand, table):
    ...
    highest_hand_combinations = [hand[1] for hand in evaluated_hands if hand[0] == highest_hand_rank]
    ...
    if len(highest_hand_combinations) == 1:
        ...
        best_hand_combination = max(
            ...
            key=lambda hand: [
                rank_values([card.rank])[0] for card in sorted(hand, key=lambda x: rank_values([x.rank])[0], reverse=True)
            ]
        )
